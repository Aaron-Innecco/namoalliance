...
export default function CouncilDiscussion() {
  ...
  const fetchComments = async () => {
    try {
      const response = await fetch(`${API_BASE}/api/council_discussion`);
  ...
  const handleLogin = async (e) => {
    e.preventDefault();
    const response = await fetch(`${API_BASE}/api/auth/check_council_password`, {
      method: 'POST',
  ...
  const handleReply = async (e, commentId) => {
    ...
    if (rMessage) {
      const response = await fetch(
        `${API_BASE}/api/council_discussion/${commentId}/reply`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
  ...
  if (!authenticated) {
    return (
      <form onSubmit={handleLogin} className="main-content">
        <h2>Enter Council Password to Access Council Discussion</h2>
        <input
          type="password"
          value={inputPassword}
  ...
  return (
    <div className="main-content">
      <h2>Council Discussion Board</h2>
      <h3>
